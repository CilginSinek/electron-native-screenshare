...
let cachedLinuxBackend = null;
let isBackendProbed = false;

/**
 * Returns the active Linux audio backend (PulseAudio, PipeWire, JACK, ALSA) by probing pactl.
 * Result is cached so it doesn't spawn processes on subsequent calls.
 * @returns {string|null}
 */
function getLinuxAudioBackend() {
    if (platform !== 'linux') return null;
    if (isBackendProbed) return cachedLinuxBackend;
    
    isBackendProbed = true;
    try {
        const pactlOut = execSync('pactl info', { stdio: ['pipe', 'pipe', 'ignore'], encoding: 'utf8' });
        if (pactlOut.includes('PipeWire')) {
            cachedLinuxBackend = 'PipeWire';
        } else if (pactlOut.includes('PulseAudio')) {
            cachedLinuxBackend = 'PulseAudio';
        }
    } catch (e) {
        // pactl failed, fallback to ALSA or JACK if needed, or null
        cachedLinuxBackend = 'ALSA / JACK (or Unknown)';
    }
